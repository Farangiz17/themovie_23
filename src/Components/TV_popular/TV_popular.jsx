...
import './TV_popular.scss'

export default function TV_popular() {
    const [tvpopular, setTVPopular] = useState([]);
    const dispatch = useDispatch()
    useEffect(() => {
      async function tvpopularData() {
        await axios
          .get(
            `${process.env.REACT_APP_BASE_URL_ALL}tv/popular?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&page=1`
          )
          .then((ress) => {
            // console.log(ress, "bu ress");
            setTVPopular(ress.data.results);

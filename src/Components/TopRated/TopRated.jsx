...
export default function TopRated() {
  const [topRatedData, setData] = useState([]);
  const dispatch = useDispatch();
  useEffect(() => {
    async function topData() {
      await axios
        .get(
          `${process.env.REACT_APP_BASE_URL_ALL}movie/top_rated?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&page=1`
        )
        .then((ress) => {
          // console.log(ress);
